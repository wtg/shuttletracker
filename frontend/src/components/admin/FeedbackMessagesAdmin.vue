...
export default Vue.extend({
    ...
    methods: {
        save() {
            const myMessage = new FeedbackMessageUpdate(this.newMessage, this.newMessageVisible, new Date(), new Date());
            AdminServiceProvider.SetFeedbackMessage(myMessage).then((resp) => {
                if (resp.ok) {
                    this.success = true;
                    setTimeout(() => {
                        this.success = false;
